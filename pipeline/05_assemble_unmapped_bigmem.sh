#!/bin/bash -l
#SBATCH -p hpcc_default -N 1 -n 24 --mem 384gb --out logs/assemble_unmapped.%a_%A.log
...
do
  ...
  if [ -f $UNMAPPEDASM/$STRAIN/scaffolds.fasta ]; then
	  echo "already processed $UNMAPPEDASM/$STRAIN/scaffolds.fasta"
	  exit
  fi
  if [[ ! -s $UMAP && ! -s $UMAPSINGLE ]]; then
    echo "Need unmapped FASTQ file, skipping $STRAIN ($FILEBASE)"
     ...
     if [ -d $UNMAPPEDASM/$STRAIN ]; then
	     spades.py -o $UNMAPPEDASM/$STRAIN --continue
     else
