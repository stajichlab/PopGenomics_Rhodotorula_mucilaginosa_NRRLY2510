...
library(gdsfmt)
library(SNPRelate)
gdsfile = "plots/snps_selected-Pheno.gds"
vcf.fn <- "vcf/RmucY2510_v1.Phenoytyped.SNP.combined_selected.vcf.gz"
title="Rhodotorula mucilaginosa strains - Phenotyped"
...
pdf("plots/PCA_snp_plots-Phenotyped.pdf")
...
plot(rv$dendrogram, leaflab="none", main=title)

snpgdsDrawTree(rv, type="z-score", main=title)
snpgdsDrawTree(rv, main=title,
               edgePar=list(col=rgb(0.5,0.5,0.5, 0.75), t.col="black"))

table(rv$samp.group)
df = data.frame(group = rv$samp.group)
rownames(df) = pca$sample.id
write.csv(df,"plots/popset_inferred-Pheno.csv")
tab <- data.frame(sample.id = pca$sample.id,
                  pop = rv$samp.group,
                  EV1=pca$eigenvect[,1], # PCA vector 1
                  EV2=pca$eigenvect[,2], # PCA vector 2
